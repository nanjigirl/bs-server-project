...
def manager_data(request):
    ...
    for item in data:
        # 获取商品的每个字段，键值对形式
        x_list.append(item.ProtectionPotential)
        y_list.append(item.lastTime)
    manager_list = {
        'xData': x_list,
        'yData': y_list
    }
    # 返回json，一定要指定类型content_type='application/json'
    return HttpResponse(json.dumps({'list': manager_list}, default=DateEncoder), content_type='application/json')
